...
・MOVIESテーブルのMoviesカラムにあるGENRE（ジャンル）は基本的に自由に80文字以内で決めることができる。
　これを今回はプルダウンで決められるように設定する。プルダウン内容は以下のとおりである。

```
[アクション, サスペンス, ホラー, SF, ファンタジー, コメディ, 恋愛, ドラマ, ミステリー, その他]
```

・またGENRE（ジャンル）というのは一意に決まるわけではないため、
　空白かつ自由記述（80文字以内）が大丈夫なSUB_GENREというのも追加した。

## 開発における生成AIの活用

本アプリの開発では、生産性向上のため生成AI（gemini,Codex）を活用しました。

- 全体の仕様書をまとめて、Codexに伝えるためにGeminiを活用
- Codexに仕様書から全体のコードを作成。

**【こだわり】**
AIの生成したコードをそのまま活用するだけではなく、コードの意味を完全に理解するためにAIに
使用している関数や機能、設定ファイル等を解説してもらって、理解したうえでコードを使用しています。
また、追加・変更した機能に関しては生成AIを利用せずに己のみで機能を追加させてもらいました。
